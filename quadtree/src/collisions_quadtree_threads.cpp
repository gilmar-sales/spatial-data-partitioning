...
std::string window_title = "Collisions with QuadTree and Multithreading";

const uint particles_count = 9000;
...
// check for linking errors
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        for (uint i = 0; i < thread_count; i++) {
            threads[i] = std::thread(update_physics, &particles[thread_load*i], quad_tree, thread_load, deltaTime);
        }
